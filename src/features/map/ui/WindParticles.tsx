...
export function WindParticles({ field, map, mode }: Props) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // ⚡ Bolt: Batching all paths into a single beginPath() / stroke() call.
      // Canvas rendering is significantly faster if we avoid breaking the drawing buffer.
      // This reduces 360 individual stroke operations per frame down to 1.
      context.beginPath();
