...
export function MapLibreViewport({
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Pre-calculate origins outside transformRequest to prevent GC pressure
    // and main thread blocking on every tile request.
    const locationOrigin = globalThis.location.origin;
    const forecastOrigin = configuration.forecastHubApiBaseUrl
      ? new URL(configuration.forecastHubApiBaseUrl, locationOrigin).origin
      : null;
    ...
    const map = new Map({
      ...
      transformRequest: (url) => {
        ...
        if (new URL(url, locationOrigin).origin !== forecastOrigin) return { url };
