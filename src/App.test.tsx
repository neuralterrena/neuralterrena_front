...
describe("App", () => {
  ...
  it("authenticates and routes into the private console", async () => {
    vi.spyOn(globalThis, "fetch")
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ access: createAccessToken("usr_console") }), { status: 200 }));

    render(<App />);

    fireEvent.change(await screen.findByLabelText("Correo"), { target: { value: "operator@neuralterrena.com" } });
    fireEvent.change(screen.getByLabelText("Clave"), { target: { value: "secret" } });
  ...
  it("restores the authenticated session on load using refresh", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ access: createAccessToken("usr_bootstrap") }), {
        status: 200,
      }),
    );
  ...
  it("navigates to forgot password and submits the reset request", async () => {
    const fetchSpy = vi
      ...
      .mockResolvedValueOnce(new Response(null, { status: 401 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ detail: "ok" }), { status: 200 }));

    render(<App />);

    fireEvent.click(await screen.findByRole("link", { name: "¿Olvidaste tu clave?" }));
    fireEvent.change(await screen.findByLabelText("Correo"), { target: { value: "operator@neuralterrena.com" } });
    fireEvent.click(screen.getByRole("button", { name: "Enviar enlace" }));

    await waitFor(() => {
      expect(screen.getByRole("status")).toHaveTextContent("Si la cuenta existe, se ha enviado un correo de recuperación.");
    });
  ...
  it("submits the password reset confirmation screen", async () => {
    ...
    window.history.replaceState({}, "", "/reset-password?uid=uid-1&token=token-1");
    render(<App />);

    fireEvent.change(await screen.findByLabelText("Nueva clave"), { target: { value: "new-secret" } });
    fireEvent.change(screen.getByLabelText("Confirmar clave"), { target: { value: "new-secret" } });
    fireEvent.click(screen.getByRole("button", { name: "Actualizar clave" }));

    await waitFor(() => {
      expect(screen.getByRole("status")).toHaveTextContent("La clave se actualizó. Inicia sesión con las nuevas credenciales.");
    });
  });
  ...
  it("opens the user menu in the dashboard and signs out from there", async () => {
    vi.spyOn(globalThis, "fetch")
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ detail: "Logout completed successfully." }), { status: 200 }))
      .mockResolvedValueOnce(new Response(null, { status: 401 }));
