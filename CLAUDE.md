...
`DESIGN.MD` is current: it was regenerated from the Neural Terrena Design System canon (Claude Design, August 2026), and `src/styles/tokens.scss` is a direct port of that system's `colors_and_type.css`. Of the canon's three product surfaces, **Map Controls and the Command View shell + timeline are implemented**; Terrain Intelligence is not, nor are the Command View's alerts and rules.
